...
onSurfacePrepared(@NonNull CameraCaptureSession session, @NonNull Surface surface); Surface准备就绪
    * */
    private final CameraCaptureSession.StateCallback mSessionCallback
        ...
        public void onConfigured(@NonNull CameraCaptureSession session) {
            ...
            updateAutoFocus();// 获取变焦信息
            updateFlash();//更新 闪光灯信息
            try {
                /*可以 持续的进行预览了*/
    ...
    /*Camera Capture Session 摄像头捕获会话 */
    PictureCaptureCallback mCaptureCallback = new PictureCaptureCallback() {
        ...
        public void onPrecaptureRequired() {
             ...
             * 描述对焦 和 曝光区域
             * 控制预触发 CONTROL_AE_PRECAPTURE_TRIGGER
             * 触发控制装置启动 CONTROL_AE_PRECAPTURE_TRIGGER_START
             */
            mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER,
                    CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER_START);
            setState(STATE_PRECAPTURE);
            try {
                /*capture 提交一个请求，请求相机设备捕获图像*/
                /*this  mCaptureCallback */
                mCaptureSession.capture(mPreviewRequestBuilder.build(), this, null);

                mPreviewRequestBuilder.set(CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER,
                        CaptureRequest.CONTROL_AE_PRECAPTURE_TRIGGER_IDLE);
                /*CONTROL_AE_PRECAPTURE_TRIGGER_IDLE  控制预触发器空闲*/
            } catch (CameraAccessException e) {
    ...
    /*摄像头捕获会话  开发者通过创建CaptureRequest向摄像头发起Capture请求，这些请求会排成一个队列供摄像头处理，摄像头将结果包装在CaptureMetadata中返回给开发者。整个流程建立在一个CameraCaptureSession的会话中。*/
   public CameraCaptureSession mCaptureSession;
    ...
    public boolean start() {
        if (!chooseCameraIdByFacing()) {//不能用 tureview 为false的时候 使用 Surfaceview
            return false;
        }
        collectCameraInfo();
        prepareImageReader();
    ...
    void startCaptureSession() {
        ...
        Surface surface = mPreview.getSurface();//此时surface 不为空
        try {
            /**
            ...
            /*addTarget  添加多个Surface 无效,Surface 必须 最近使用的 */
        ...
        try {
            ...
            captureRequestBuilder.set(CaptureRequest.JPEG_ORIENTATION,
                    (sensorOrientation +
                            ...
                            360) % 360);
            // Stop preview and capture a still picture.    //停止连续取景
