...
import com.base.common.file.FileUtil;
...
import com.base.hyl.houylbaseprojects.download.core.core.DownloadResponse;
import com.base.hyl.houylbaseprojects.download.core.core.DownloadResponseImpl;
import com.base.hyl.houylbaseprojects.download.core.core.DownloadTaskImpl;
import com.base.hyl.houylbaseprojects.download.core.core.task.DownloadTask;
import com.base.hyl.houylbaseprojects.download.core.db.DefaultDownloadDBController;
...
public final class DownloadManagerImpl implements DownloadManager, DownloadTaskImpl.DownloadTaskListener {
    ...
    private DownloadManagerImpl(Context context, Config config) {
        ...
        MyLog.wtf(TAG,"findAllDownloading 初始化 DownloadManagerImpl"+downloadingCaches.size());
...
//        if (cacheDownloadTask.size()*config.getEachDownloadThread() >= config.getDownloadThread()) {
    ...
    @Override
    public void onDestroy() {/* 正在下载的内容 暂停下载*/
        for (DownloadInfo downloadInfo : downloadingCaches) {
            if(downloadInfo.getStatus()==DownloadInfo.STATUS_WAIT||
                    downloadInfo.getStatus()==DownloadInfo.STATUS_DOWNLOADING||
                    downloadInfo.getStatus()==DownloadInfo.STATUS_PREPARE_DOWNLOAD
            ){
                pauseInner(downloadInfo);
            }
        }
