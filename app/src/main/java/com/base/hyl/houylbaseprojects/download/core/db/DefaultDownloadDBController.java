...
import static com.base.hyl.houylbaseprojects.download.core.domain.DownloadInfo.STATUS_PAUSED;
...
public final class DefaultDownloadDBController implements DownloadDBController {
    ...
    public static final String[] DOWNLOAD_INFO_COLUMNS = new String[]{"_id", "supportRanges",
            "createAt", "uri",
            "path", "size", "progress",
            "status","imgUrl","title","description","isSelected","isBanner"};
    ...
    public static final String SQL_UPDATE_DOWNLOAD_INFO = String.format(
            "REPLACE INTO %s (_id,supportRanges,createAt,uri,path,size,progress,status,imgUrl,title,description,isSelected,isBanner) VALUES(?,?,?,?,?,?,?,?,?,?,?,?,?);",
    ...
    private void inflateDownloadInfo(Cursor cursor, DownloadInfo downloadInfo) {
        ...
        downloadInfo.setImgUrl(cursor.getString(8));
        downloadInfo.setTitle(cursor.getString(9));
        downloadInfo.setDescription(cursor.getString(10));
        downloadInfo.setIsSelected(cursor.getInt(11));
        downloadInfo.setIsBanner(cursor.getInt(12));
    }
    ...
    @Override
    public DownloadInfo findDownloadedInfoByUri(String uri) {
        Cursor cursor = readableDatabase
                .query(DefaultDownloadHelper.TABLE_NAME_DOWNLOAD_INFO, DOWNLOAD_INFO_COLUMNS, "uri=?",
                        new String[]{uri},
                        null, null, "createAt desc");
        if (cursor.moveToNext()) {
            DownloadInfo downloadInfo = new DownloadInfo();
    ...
    public void createOrUpdate(DownloadInfo downloadInfo) {
        writableDatabase.execSQL(
                ...
                new Object[]{
                        ...
                        downloadInfo.getImgUrl(), downloadInfo.getTitle(), downloadInfo.getDescription(),
                        downloadInfo.getIsSelected(), downloadInfo.getIsBanner()});
    }
